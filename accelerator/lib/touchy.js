...
(function() {
  ...
  var d = document,
      ...
      msTouchAction = true, // true - add to elements touch action:none
  ...
  function onStart (event) {
    ...
    if (typeof event.target.style.msTouchAction !== 'undefined' && msTouchAction) event.target.style.msTouchAction = 'none';
  ...
  // start adding ms-touch-action to elements after action on it
  function startMsTouch() {
    msTouchAction = true;
  }

  // stop adding ms-touch-action to elements
  function stopMsTouch() {
    msTouchAction = false;
  }
  ...
  window.touchy = {
    isTouch: isTouch,
    stop: stopBubbling,
    stopMsTouch: stopMsTouch,
    startMsTouch: startMsTouch,
    events: evts,
    version: "1.1.1"
  };
}());