...
(function () {
  ...
  function getApiInterface(client) {
    let engagerWinApi = "http://engager.agnitio.com";
    let engagerIosApi = "objc://iplanner";
    switch (client) {
      case "engager-ios":
        return engagerIosApi;
      case "engager-win":
        return engagerWinApi;
      default:
        return null;
    }
  }

  function parseAgnitioInfo(client, version) {
    let platform;
    switch (client) {
      case "engager-ios":
        platform = 'iOS'
  ...
  var api_version = '1.6.4',
    customInvoke = false,
    client = getURLQueryParameterByName('client'),
    version = getURLQueryParameterByName('version'),
    appInterface = null;

  if (client && version) {
    appInterface = getApiInterface(client);
    window.agnitioInfo = parseAgnitioInfo(client,version)
  }
  appInterface = getApiInterface(client);
  ...
  ag.debug = function(writeToConsole) {
    ...
    var meToken, pdfToken, urlToken, mailToken, captureToken,
        presenterToken, contactsToken, attributesToken,
        provideContent, sendContent, proxyContent, callDetailsContacts ;
    ...
    function start () {
      ...
      sendContent = ag.on('sendContent', log);
      proxyContent = ag.on('proxyContent', log);
      provideContent = ag.on('provideContent', log);
      ...
      callDetailsContacts = ag.on('callDetailsContacts', function(params) {write('callDetailsContacts', params)} );
    }
  ...
  ag.platform = (function() {
    ...
    function info () {
      if (window.agnitioInfo) return window.agnitioInfo;
      return undefined;
    }

    // Utility for quickly checking if currently running in Agnitio
    function isAgnitio () {
      if (window.agnitioInfo) return true;
      return false;
    }
    ...
    function isApp () {
      let client = getURLQueryParameterByName('client');
      if (client === 'engager-ios' || client === 'engager-win' ) return true;
      // Below line is for legacy compatibility. To be removed after all clients send "client" in query params
      if ( /(iPhone|iPod|iPad).*AppleWebKit(?!.*Version)/i.test(navigator.userAgent)) return true;
      return false;
    }
  ...
  ag.remote = (function() {
    ...
    // Register events for provisioning in remote
    ag.on("sendContent", function(event) {
      ag.msg.send({
        name: "sendContent",
        value: JSON.stringify(event)
      })
    ...
    ag.on("proxyContent", function(event) {
      ag.msg.send({
        name: "proxyContent",
        value: JSON.stringify(event)
      })
    ...
    ag.on("provideContent", function(event) {
      ag.msg.send({
        name: "provideContent",
        value: JSON.stringify(event)
      })
    ...
    ag.on('getCallContacts',function(event){
      ag.msg.send({
        name: "getCallContacts",
        value: JSON.stringify(event)
      })
    })

    ag.on('callDetailsContacts',function(event){
      if (event.contacts && event.contacts.length)
        ag.data._saveContacts(event.contacts);
    });
  ...
  ag.submit = (function() {
    ...
    /**
     * Removes whitespace from both ends of Object values of type string.
    ...
    function sanitize(data) {
      if (data) {
        ...
        keys.forEach(function (key) {
          ...
          } catch (err) {/* Do nothing when it is not a valid string, e.g "Int", "undefined" or "null" */}
        });
        return data;
      }
      return data;
    ...
    function save (data) {
      if (isEnabled()) {
        var monitoringData = sanitize(data);
        invoke('monitoringEvent', JSON.stringify(monitoringData));
        publish('monitoringEvent', monitoringData);
      }
    }
  ...
  function receiveMessage(event) {
    ...
    if (event.data) {
      ...
      switch (event.data.name) {
        ...
        case "register":
          // Send event so that frameworks/presentations can act on it
          publish('registerUser', event.data);
        ...
        case "connect":
          ...
          break;
        case "callDetailsContacts":
          publish("callDetailsContacts",event.data);
          break;
